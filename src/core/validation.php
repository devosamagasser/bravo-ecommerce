...
namespace Bravo\Store\core;
    ...
    class validation extends Exception {
            ...
            if(!empty($img['name']) ){
                if( $img['error'] == 0 ){
                    ...
                    if(in_array($img_ext,$ava_ext)){
                        if($img['size'] <= 2000000){
                            $this -> value = md5(uniqid()).".png";
                            move_uploaded_file($img['tmp_name'],"D:\\xampp\htdocs\startbootstrap-freelancer-gh-pages\public\img\\{$this -> value}");
                        }else{ 
        ...
        public function fImages()
        {
            ...
            $images = [] ;
            if(empty($img['name'][0]) ){
                throw new Exception("Please Chose Images");
            }
            for($i = 0;$i < $count ;$i++ ){
                if(!empty($img['name'][$i]) ){
                    if( $img['error'][$i] == 0 ){
                        $ava_ext = ['jpg','png','jfif','gif','jpeg','webp'];
                        $img_ext = pathinfo($img['name'][$i],PATHINFO_EXTENSION);
                        if(in_array($img_ext,$ava_ext)){
                            if($img['size'][$i] <= 2000000){
                                $images[] = md5(uniqid()).".webp";
                                move_uploaded_file($img['tmp_name'][$i],"D:\\xampp\htdocs\bravo\public\img\\".$images[$i]);
                            }else{ 
